				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		hphrocp_off_report(mbhc, SND_JACK_OC_HPHR);
		hphlocp_off_report(mbhc, SND_JACK_OC_HPHL);
		hs_type = mbhc->current_plug = MBHC_PLUG_TYPE_NONE;
	} else {
		/*
		...
		if (jack_type == SND_JACK_HEADPHONE)
			hs_type = mbhc->current_plug = MBHC_PLUG_TYPE_HEADPHONE;
		else if (jack_type == SND_JACK_UNSUPPORTED)
			hs_type = mbhc->current_plug = MBHC_PLUG_TYPE_GND_MIC_SWAP;
		else if (jack_type == SND_JACK_HEADSET) {
			hs_type = mbhc->current_plug = MBHC_PLUG_TYPE_HEADSET;
			mbhc->jiffies_atreport = jiffies;
		}
		else if (jack_type == SND_JACK_LINEOUT)
			hs_type = mbhc->current_plug = MBHC_PLUG_TYPE_HIGH_HPH;
...
/* zte weizhijun add headset detect /proc/hs 2013-12-17 start */
static int hs_read(struct file *file, char __user *buf,
