...
#endif
{
	...
	tlen = sprintf(buf , "[%02d-%02d %02d:%02d:%02d.%03d] ",
		...
		tm.tm_sec, (int)(rem_nsec / NSEC_PER_MSEC));
...
#else
	len += print_time(msg->ts_nsec, buf ? buf + len : NULL, msg->process_id, msg->pid, msg->comm);
#endif
	return len;
}
...
#else
		textlen += print_time(cont.ts_nsec, text,  cont.process_id, cont.pid, cont.comm);
#endif
		size -= textlen;
	}
