...
#if defined(ZTE_FEATURE_TF_SECURITY_SYSTEM)
#if defined(CONFIG_ZTE_KMOD_AUTH_SEC)
int pil_init_image_trusted_kmod_auth_sec(const u8 *metadata, size_t size)
{
	struct pil_tz_data data;
	struct pil_desc *pil = &data.desc;

	memset((void *)&data, 0, sizeof(struct pil_tz_data));
	data.pas_id = PAS_KMOD_AUTH_SEC;

	return pil_init_image_trusted(pil, metadata, size);
}
#endif /* CONFIG_ZTE_KMOD_AUTH_SEC */
#endif /* ZTE_FEATURE_TF_SECURITY_SYSTEM */
