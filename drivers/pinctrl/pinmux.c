		       ...
		       struct pinctrl_gpio_range *gpio_range)
{
	...
	if (gpio_range) {
		...
		if (desc->gpio_owner) {
			dev_err(pctldev->dev,
				"gpio mode, pin %s already requested by %s; cannot claim for %s\n",
		...
		if (desc->mux_usecount && strcmp(desc->mux_owner, owner)) {
			...
			//goto out;		//zte remove for cameras use the same gpio
		}

		desc->mux_usecount++;
#if 0		//zte remove for cameras use the same gpio
