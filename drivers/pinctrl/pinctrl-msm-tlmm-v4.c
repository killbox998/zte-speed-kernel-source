...
//zte jiangfeng add
static int msm_tlmm_gp_cfg_get(uint pin_no, struct msm_pintype_info *pinfo)
{
...
//zte jiangfeng add
static int msm_tlmm_v4_gp_fn_get(uint pin_no, struct msm_pintype_info *pinfo)
{
	unsigned int val;
	void __iomem *cfg_reg = TLMMV4_GP_CFG(pinfo->reg_base, pin_no);
	val = readl_relaxed(cfg_reg);
	val >>= TLMMV4_GP_FUNC_SHFT;
	val &= TLMMV4_GP_FUNC_MASK;
	return val;
}
//zte jiangfeng add, end
...
//zte jiangfeng add
static int msm_tlmm_gp_in_get(uint pin_no, struct msm_pintype_info *pinfo)
...
static int msm_tlmm_gp_out_get(uint pin_no, struct msm_pintype_info *pinfo)
{
	void __iomem *inout_reg = TLMMV4_GP_INOUT(pinfo->reg_base, pin_no);
...
//zte jiangfeng add
static int msm_tlmm_gp_dir_get(uint pin_no, struct msm_pintype_info *pinfo)
{
	unsigned int val;
	void __iomem *cfg_reg = TLMMV4_GP_CFG(pinfo->reg_base, pin_no);
...
static int msm_tlmm_gp_dir_set(uint pin_no, struct msm_pintype_info *pinfo, u64 value)
{
	unsigned int val;
	void __iomem *cfg_reg = TLMMV4_GP_CFG(pinfo->reg_base, pin_no);
...
//zte jiangfeng add
static int msm_tlmm_set_intr_cfg_get(uint pin_no, struct msm_pintype_info *pinfo)
{
	unsigned int val;
	void __iomem *cfg_reg = TLMMV4_GP_INTR_CFG(pinfo->reg_base, pin_no);
...
static int gpio_debug_direction_set(void *data, u64 val)
{
	...
	msm_tlmm_gp_dir_set(*id, tlmm_v4_pininfo, val);
	return 0;
}
...
{
	int *id = data;
	*val = 	msm_tlmm_gp_dir_get(*id, tlmm_v4_pininfo);
...
static int gpio_debug_level_set(void *data, u64 val)
{
	...
	msm_tlmm_v4_gp_set(pintype_get_gc(tlmm_v4_pininfo), *id, val);
...
{
	int *id = data;
	int dir = msm_tlmm_gp_dir_get(*id, tlmm_v4_pininfo);

	if (dir)
		*val = msm_tlmm_gp_out_get(*id, tlmm_v4_pininfo);
	else
		*val = msm_tlmm_gp_in_get(*id, tlmm_v4_pininfo);
...
static int gpio_debug_drv_set(void *data, u64 val)
{
	...
	int drv_reg;
	void __iomem *cfg_reg = TLMMV4_GP_CFG(tlmm_v4_pininfo, *id);
	...
	if(drv_reg > 7)
		drv_reg	=	7;
	reg_val &= ~(TLMMV4_GP_DRV_MASK << TLMMV4_GP_DRV_SHFT);
	reg_val |= drv_reg << TLMMV4_GP_DRV_SHFT;
...
static int gpio_debug_drv_get(void *data, u64 *val)
{
	...
	reg_val = readl_relaxed(cfg_reg);
	drv_reg = (reg_val >> TLMMV4_GP_DRV_SHFT) & TLMMV4_GP_DRV_MASK;
	*val = (drv_reg + 1)*2;
	return 0;
}
...
static int gpio_debug_func_sel_set(void *data, u64 val)
{
	...
	msm_tlmm_v4_gp_fn(*id, val, tlmm_v4_pininfo, true);
	return 0;
}
...
static int gpio_debug_func_sel_get(void *data, u64 *val)
{
	...
	*val = msm_tlmm_v4_gp_fn_get(*id, tlmm_v4_pininfo);
	return 0;
}
...
static int gpio_debug_pull_set(void *data, u64 val)
{
	...
	reg_val = readl_relaxed(cfg_reg);
	reg_val &= ~(TLMMV4_GP_PULL_MASK << TLMMV4_GP_PULL_SHFT);
	reg_val |= val << TLMMV4_GP_PULL_SHFT;
...
static int gpio_debug_pull_get(void *data, u64 *val)
{
	...
	reg_val = readl_relaxed(cfg_reg);
	*val = (reg_val >> TLMMV4_GP_PULL_SHFT) & TLMMV4_GP_PULL_MASK;
	return 0;
}
...
static int gpio_debug_int_enable_get(void *data, u64 *val)
{
	...
	*val = msm_tlmm_v4_get_intr_cfg_enable(pintype_get_ic(tlmm_v4_pininfo), *id);
...
static int gpio_debug_int_owner_set(void *data, u64 val)
{
	...
	void __iomem *cfg_reg = TLMMV4_GP_INTR_CFG(tlmm_v4_pininfo, *id);
	reg_val = readl_relaxed(cfg_reg);
...
static int gpio_debug_int_owner_get(void *data, u64 *val)
{
	...
	int_cfg = msm_tlmm_set_intr_cfg_get(*id,tlmm_v4_pininfo);
...
int msm_dump_gpios(struct seq_file *m, int curr_len, char *gpio_buffer)
{
	...
	for (i = 0; i < pinfo->num_pins; i++) {
		...
		func_sel	=	(cfg >> TLMMV4_GP_FUNC_SHFT) & TLMMV4_GP_FUNC_MASK;
		...
		pull = (cfg >> TLMMV4_GP_PULL_SHFT) & TLMMV4_GP_PULL_MASK;
		switch (pull) {
		case 0x0:
		...
		drv = (cfg >> TLMMV4_GP_DRV_SHFT) & TLMMV4_GP_DRV_MASK;
...
static int msm_tlmm_v4_probe(struct platform_device *pdev)
{
	...
	gpio_status_debug_init(tlmm_v4_pininfo);		//zte jiangfeng add
	return result;
}
