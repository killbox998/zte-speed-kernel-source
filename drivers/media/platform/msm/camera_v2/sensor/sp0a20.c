...
static struct msm_sensor_power_setting sp0a20_power_setting[] = {
	...
	{
    		.seq_type = SENSOR_GPIO,
    		.seq_val = SENSOR_GPIO_VDIG_OTHER,
    		.config_val = GPIO_OUT_HIGH,
    		.delay = 5,
  	},	
...
static int32_t sp0a20_i2c_write_table(struct msm_sensor_ctrl_t *s_ctrl,
		struct msm_camera_i2c_reg_conf *table,
		int num)
{
	int32_t rc = 0;
	rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
			i2c_write_conf_tbl(s_ctrl->sensor_i2c_client,table,
					num,MSM_CAMERA_I2C_BYTE_DATA);
	return rc;
...
static void sp0a20_set_stauration(struct msm_sensor_ctrl_t *s_ctrl, int value)
{
	...
	rc = sp0a20_i2c_write_table(s_ctrl, &sp0a20_reg_saturation[value][0],
...
static void sp0a20_set_contrast(struct msm_sensor_ctrl_t *s_ctrl, int value)
{
	int32_t rc = 0;
	pr_debug("%s %d", __func__, value);
	rc = sp0a20_i2c_write_table(s_ctrl, &sp0a20_reg_contrast[value][0],
...
static void sp0a20_set_sharpness(struct msm_sensor_ctrl_t *s_ctrl, int value)
{
	...
	int val = value / 6;
	pr_debug("%s %d", __func__, value);
	rc = sp0a20_i2c_write_table(s_ctrl, &sp0a20_reg_sharpness[val][0],
...
static void sp0a20_set_iso(struct msm_sensor_ctrl_t *s_ctrl, int value)
{
	int32_t rc = 0;
	pr_debug("%s %d", __func__, value);
	rc = sp0a20_i2c_write_table(s_ctrl, &sp0a20_reg_iso[value][0],
	...
	int value)
{
	...
	rc = sp0a20_i2c_write_table(s_ctrl, &sp0a20_reg_exposure_compensation[val][0],
...
static void sp0a20_set_effect(struct msm_sensor_ctrl_t *s_ctrl, int value)
{
	...
	if(rc < 0)
		pr_err("%s  value = %d fail", __func__,value);
}
...
static void sp0a20_set_antibanding(struct msm_sensor_ctrl_t *s_ctrl, int value)
{
	int32_t rc = 0;
	pr_debug("%s %d", __func__, value);
	rc = sp0a20_i2c_write_table(s_ctrl, &sp0a20_reg_antibanding[value][0],
	...
	int value)
{
	...
	case MSM_CAMERA_WB_MODE_AUTO: {
		rc = sp0a20_i2c_write_table(s_ctrl, &sp0a20_reg_wb_auto[0],
				ARRAY_SIZE(sp0a20_reg_wb_auto));
		break;
	}
	case MSM_CAMERA_WB_MODE_INCANDESCENT: {
		rc = sp0a20_i2c_write_table(s_ctrl, &sp0a20_reg_wb_office[0],
				ARRAY_SIZE(sp0a20_reg_wb_office));
		break;
	}
	case MSM_CAMERA_WB_MODE_DAYLIGHT: {
		rc = sp0a20_i2c_write_table(s_ctrl, &sp0a20_reg_wb_sunny[0],
				ARRAY_SIZE(sp0a20_reg_wb_sunny));
		break;
	}
	case MSM_CAMERA_WB_MODE_FLUORESCENT: {
		rc = sp0a20_i2c_write_table(s_ctrl, &sp0a20_reg_wb_home[0],
				ARRAY_SIZE(sp0a20_reg_wb_home));		
		break;
	}
	case MSM_CAMERA_WB_MODE_CLOUDY_DAYLIGHT: {
		rc = sp0a20_i2c_write_table(s_ctrl, &sp0a20_reg_wb_cloudy[0],
