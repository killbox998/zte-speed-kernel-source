 ...
 */
/* ZTE-MODIFY BY jiangzhineng, add FEATURE_KEEP_SOC_FULL_IF_USB_PRESENT if needed */
#define FEATURE_KEEP_SOC_FULL_IF_USB_PRESENT    0
/* ZTE-MODIFY BY jiangzhineng, add temperature protection */
#define FEATURE_BATT_TEMP_CONTROL                                 0
#define KEEP_AWAKE_WHEN_CHARGING                                  1
...
#include <linux/wakelock.h>    /* zte add for wakelock */
...
#define INPUT_CURR_LIM_500MA		0x3
...
#define CHG_TEMP_COMP			0x14    //defined by zte
#define CHG_HOT_COLD_COPM_MASK		SMB1360_MASK(7, 4)
#define HOT_VOLT_COMP_ENABLE_BIT		BIT(7)
#define COLD_VOLT_COMP_ENABLE_BIT	BIT(6)
#define HOT_CURR_COMP_ENABLE_BIT	BIT(5)
#define COLD_CURR_COMP_ENABLE_BIT	BIT(4)

#define CHG_CMP_CFG			0x14        //defined by qualcomm    
...
#define FG_RESET_THRESHOLD_MV		400 // zte set to 400mV
...
//static int otg_curr_ma[] = {350, 550, 950, 1500};  20140905 baseline
...
struct smb1360_chip {
	...
	bool				irq_waiting;
	bool				empty_soc;
	#if FEATURE_KEEP_SOC_FULL_IF_USB_PRESENT
	bool				usb_present_after_batt_full;
	#endif
...
static int smb1360_enable_fg_access(struct smb1360_chip *chip)
{
	...
	if (!timeout){
		pr_err(" error:smb1360_enable_fg_access timeout\n");
		return -EBUSY;
	}
...
/* ZTE-MODIFY BY jiangzhineng for disable usb charging and disable shutdown low battery :begin*/
...
/* ZTE-MODIFY BY jiangzhineng for disable usb charging and disable shutdown low battery:end*/
...
/**** zte jiangzhineng add; end  ****/
	...
	if(report_zero){		
		pr_err("kernel is waiting for APP shutdown,as SOC==0. \n");		
		return 0;		
	}  //zte jzn add
    ...
    /* ZTE-MODIFY BY jiangzhineng: begin*/

        battery_v = smb1360_get_prop_voltage_now(chip);
	
        if ((first_time_run>=1)&&(battery_v>3200000))	{			
	         pr_err("batt voltage = %d,first_time_run=%d,soc=%d%%\n", battery_v,first_time_run,soc);			
		first_time_run--;			
		if (soc <= 3){				
			if(soc==0)				
				soc=1;				
	...
	{			
		pr_err("batt capacity = %d%%,voltage=%d,double_check=%d\n", soc,battery_v,checkflag);			
		//#if defined(CONFIG_MACH_HAYES)||defined(CONFIG_MACH_ELDEN) //ZTE_XJB_PM_20120831 start			
		if(checkflag>=10){				
			//double_check=0;				
			soc = 0;				
			pr_err("batt percent_soc seting 0 here as voltage and no charging\n");			
		}			
		else	{				
			checkflag++;			
	...
	if (soc <= 5)			
		pr_err("ZTE  soc=%d,g_bms_flag=%d,flag_check=%d ,count_number=%d\n",soc,g_bms_flag,flag_check,count_number);
	...
	if(soc==0)
	{
		pr_err( "get_prop_batt_capacity: HAHA shutdown as percent_soc=%d ,report_zero =%d\n",  soc,report_zero);
		report_zero=true;
	}
	else
		report_zero=false;	
	
    /* ZTE-MODIFY BY jiangzhineng: end*/
	pr_err("msys_soc_reg=0x%02x, fg_soc=%d batt_full = %d\n", reg,
						soc, chip->batt_full);
	
#if FEATURE_KEEP_SOC_FULL_IF_USB_PRESENT
	pr_err("chip->usb_present_after_batt_full = %d, chip->usb_present = %d, chip->batt_full = %d\n",chip->usb_present_after_batt_full,chip->usb_present,chip->batt_full);
	return ((chip->usb_present_after_batt_full && chip->usb_present) || chip->batt_full) ? 100 : bound(soc, 0, 100);
#else
	if(!chip->batt_full && soc==100) soc=99; //zte jiangzhineng add
...
static int smb1360_set_minimum_usb_current(struct smb1360_chip *chip)
{
	...
	pr_err("set USB current to minimum\n");
	/* set input current limit to minimum (500mA)*/
	rc = smb1360_masked_write(chip, CFG_BATT_CHG_ICL_REG,
					INPUT_CURR_LIM_MASK,
					INPUT_CURR_LIM_500MA);
	if (rc)
...
static int smb1360_set_appropriate_usb_current(struct smb1360_chip *chip)
{
	...
	if (i < 0) {
		pr_err("Couldn't find ICL mA rc=%d\n", rc);
		i = 0;
	}
	...
	if ((current_ma <= CURRENT_100_MA) &&
		...
		pr_err("usb100 not supported, set current to 500mA\n");
		current_ma = CURRENT_500_MA;
	}

	if (current_ma <= CURRENT_100_MA) {
		/* USB 100 */
		/* changed by zte jiangzhineng,set to 500*/
		rc = smb1360_masked_write(chip, CMD_IL_REG,
				USB_CTRL_MASK, USB_500_BIT);
		if (rc)
			pr_err("Couldn't configure for USB100 rc=%d\n", rc);
		pr_err("Setting USB 500\n");
		...
		if (i < 0) {
			pr_err("Couldn't find fastchg mA rc=%d\n", rc);
			i = 0;
		}
		/* set fastchg limit */
		...
		pr_err("fast-chg current set to = %d\n", fastchg_current[i]);
	}
...
static void smb1360_external_power_changed(struct power_supply *psy)
{
	...
	else
	{
		...
		/* ZTE-MODIFY BY jiangzhineng,set current_limit=1050,so ICL=1000, ICHG=1050 */
		if(current_limit >= 1050)
			current_limit = 1050;
	}
...
#if 0	
 	int rc=0;
/* ZTE-MODIFY BY jiangzhineng,change low temperature voltage of irq_volt_empty:BEGIN*/
// if temperature<=0,change the voltage of  irq_voltage_empty to 3V(0x4c),otherwise set to 3.4V(0X58,default)
	if(chip->batt_cold)
	{
		rc = smb1360_write(chip, VTG_EMPTY_REG, 0x4C);
		if (rc < 0)
			dev_err(chip->dev,
				"Couldn't set VTG_EMPTY_REG rc=%d\n", rc);
		
		rc = smb1360_enable_fg_access(chip);
		if (rc)
			pr_err("Couldn't enable FG access rc=%d\n", rc);
		rc = smb1360_write(chip, FG_SYS_CUTOFF_V_REG, 0xCC);
		rc = smb1360_write(chip, FG_SYS_CUTOFF_V_REG+1, 0x4C); //set to 3V
		if (rc < 0)
			dev_err(chip->dev,
				"Couldn't set FG_SYS_CUTOFF_V_REG rc=%d\n", rc);
		smb1360_disable_fg_access(chip);
	}
	else
	{
		rc = smb1360_write(chip, VTG_EMPTY_REG, 0x58);
		if (rc < 0)
			dev_err(chip->dev,
				"Couldn't set VTG_EMPTY_REG rc=%d\n", rc);
		...
		if (rc)
			pr_err("Couldn't enable FG access rc=%d\n", rc);
		rc = smb1360_write(chip, FG_SYS_CUTOFF_V_REG, 0x0B);
		rc = smb1360_write(chip, FG_SYS_CUTOFF_V_REG+1, 0x57);//set to 3.4V
		if (rc < 0)
			dev_err(chip->dev,
				"Couldn't set FG_SYS_CUTOFF_V_REG rc=%d\n", rc);
		smb1360_disable_fg_access(chip);
	}
/* ZTE-MODIFY BY jiangzhineng,change low temperature voltage of irq_volt_empty:END*/	
#endif
...
static int chg_term_handler(struct smb1360_chip *chip, u8 rt_stat)
{
	...
	#if FEATURE_KEEP_SOC_FULL_IF_USB_PRESENT
	if(chip->usb_present && chip->batt_full){
		pr_err("set chip->usb_present_after_batt_full = true\n");
		chip->usb_present_after_batt_full = true;
	}
	#endif
...
static int usbin_uv_handler(struct smb1360_chip *chip, u8 rt_stat)
{
	...
	wake_lock_timeout(&chip->charger_wake_lock, 5 * HZ);     //zte jzn add
	...
	if (chip->usb_present && !usb_present) {
		...
		#if FEATURE_KEEP_SOC_FULL_IF_USB_PRESENT
		pr_err("set chip->usb_present_after_batt_full = false\n");
		chip->usb_present_after_batt_full = false;
		#endif

		#if KEEP_AWAKE_WHEN_CHARGING                   //zte jzn add
		wake_unlock(&chip->charger_fulled_lock);       
		pr_info("USB removed and unlock charger_fulled_lock !\n");
		#endif
		count_number=15;   //ZTE
		chip->batt_full  = 0;      //ZTE
	}
	...
	if (!chip->usb_present && usb_present) {
		...
		pr_info("USB inserted and ock charger_fulled_lock !\n");
		#endif
		count_number=15;   //ZTE
	}
...
/* ZTE-MODIFY BY DingXuehua jiangzhineng, add function for charger OV:begin */
...
/* ZTE-MODIFY BY DingXuehua jiangzhineng, add function for charger OV:end */
...
//ZTE--MODIFY By DingXuehua, add func
static int smb1360_check_delta_soc(struct smb1360_chip *chip)
{
	int rc = -1;
	u8 reg;
	...
	if (!rc){
		...
		if(0x02 == reg)  //delta soc interrupt flag triggered
			return 1;
	}
...
//static void smb1360_show_reg_value(struct smb1360_chip *chip);
static irqreturn_t smb1360_stat_handler(int irq, void *dev_id)
{
	...
	//ZTE--MODIFY By DingXuehua, update soc
	if(smb1360_check_delta_soc(chip))
		power_supply_changed(&chip->batt_psy);

	//smb1360_show_reg_value(chip);
	...
	if (!chip->resume_completed) {
		...
		pr_err("chip->resume_completed = 0\n");
		return IRQ_HANDLED;
	}
	...
	for (i = 0; i < ARRAY_SIZE(handlers); i++) {		
		...
		for (j = 0; j < ARRAY_SIZE(handlers[i].irq_info); j++) {			
			...
			if ((triggered || changed)
				...
				handler_count++;
				pr_info("call %pf, handler_count=%d\n",handlers[i].irq_info[j].smb_irq,handler_count);
...
#if 0
		/* ZTE-MODIFY By jiangzhineng, Enable the Auto Recharge Masking:BEGIN */
		//mask the variation on the System SoC while the charger is re-tapping the battery
		rc = smb1360_masked_write(chip, 0xED, BIT(1),BIT(1));
		if (rc < 0) {
			dev_err(chip->dev, "Couldn't set Auto Recharge Masking. rc=%d\n", rc);
			return rc;
		}
		/* ZTE-MODIFY By jiangzhineng, Enable the Auto Recharge Masking:END */
#endif		
...
{
//ZTE--MODIFY BY jiangzhineng rollback from CS SW Ver 
...
static int smb1360_hw_init(struct smb1360_chip *chip)
{
	...
	/* ZTE-MODIFY By jiangzhineng, THERM pin as battery missing pin source, enable batt missing algotithm */
	rc = smb1360_masked_write(chip, CFG_BATT_MISSING_REG,
				BATT_MISSING_ALGORITHM_EN_BIT
				|BATT_MISSING_SRC_THERM_BIT
				|BATT_MISSING_SRC_BMD_BIT,
				BATT_MISSING_ALGORITHM_EN_BIT
				|BATT_MISSING_SRC_THERM_BIT);
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't set battery missing SYSOK(0X0D) config = %d\n",
									rc);
		return rc;
	}
	
	rc = smb1360_jeita_init(chip); // LNX.LA.3.7-01330-8x16.0  (20141103)
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't init jeita, rc = %d\n", rc);
		return rc;
	}

	/* ZTE-MODIFY By jiangzhineng, enable JEITA_TEMP_HARD_ENABLED */
	rc = smb1360_masked_write(chip, CFG_FG_BATT_CTRL_REG,
				JEITA_TEMP_HARD_ENABLED_BIT,
				0x0);
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't set JEITA_TEMP_HARD_ENABLED_BIT config = %d\n",
									rc);
		return rc;
	}
	/* ZTE-MODIFY By jiangzhineng, enable JEITA_TEMP_SOFT_ENABLED */
	rc = smb1360_masked_write(chip, CHG_TEMP_COMP,
				CHG_HOT_COLD_COPM_MASK,
				HOT_VOLT_COMP_ENABLE_BIT
				|COLD_VOLT_COMP_ENABLE_BIT
				|HOT_CURR_COMP_ENABLE_BIT
				|COLD_CURR_COMP_ENABLE_BIT);		
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't set JEITA_TEMP_SOFT_ENABLED_BIT config = %d\n",
									rc);
		return rc;
	}	
...
#if 1   /* ZTE-MODIFY By DingXuehua, disable low battery blinking */
...
static void update_heartbeat(struct work_struct *work)
{
	...
	struct smb1360_chip  *chip = container_of(dwork,
				struct smb1360_chip, update_heartbeat_work);
	...
	if(chip==NULL)
	{
		pr_err("pmic fatal error:the_chip(smb1360_chip)=null\n!!");
		return;
	}
	
	temperature	=	smb1360_get_prop_batt_temp(chip)/10; 
	voltage		=	smb1360_get_prop_voltage_now(chip);
	capacity	         =	smb1360_get_prop_batt_capacity(chip);
	status		=	smb1360_get_prop_batt_status(chip);
	charge_type	=	smb1360_get_prop_charge_type(chip);
	present		=	smb1360_get_prop_batt_present(chip);
	chg_current	=	smb1360_get_prop_current_now(chip);
	pr_info( "\n update_heartbeat: battery temp = %d,voltage=%d,capacity=%d,status=%d,chg_state=%d,current=%d,present=%d\n",  temperature,voltage,capacity,status,charge_type,chg_current,present);
	power_supply_changed(&chip->batt_psy);
...
static void bms_check(struct work_struct *work)
{
	...
	struct delayed_work *dwork = to_delayed_work(work);
	struct smb1360_chip *chip = container_of(dwork,struct smb1360_chip, bms_check_work);

        if(chip==NULL)
        {
        		pr_err("smb1360 fatal error:the_chip=null\n!!");
        }
        else
	{
		//temperature  = smb1360_get_prop_batt_temp(chip)/10;
		//voltage           = smb1360_get_prop_voltage_now(chip)/1000;
		capacity	          = smb1360_get_prop_batt_capacity(chip);
		//present          = smb1360_get_prop_batt_present(chip);
		chg_current	= smb1360_get_prop_current_now(chip)/1000;
		status                 = smb1360_get_prop_batt_status(chip);
		...
		if((count_number>=30)||(count_number<=-10))
			count_number=15;
		//pr_err( "bms_check: battery temp = %d,voltage=%d,capacity=%d,current=%d,present=%d,count_number=%d\n",  temperature,voltage,capacity,chg_current,present,count_number);
		pr_err( "bms_check: battery capacity=%d,current=%d\n",  capacity,chg_current);
...
/********************* zte add end ***************/
    ...
    /* ZTE-MODIFY BY jiangzhineng for disable usb charging and disable shutdown low battery*/
	schedule_delayed_work(&chip->update_heartbeat_work,
				          round_jiffies_relative(msecs_to_jiffies
						(HEARTBEAT_MS)));
    the_chip = chip;
...
static int smb1360_suspend(struct device *dev)
{
	...
	rc = smb1360_write(chip, IRQ_CFG_REG, IRQ_DCIN_OV_BIT |IRQ_DCIN_UV_BIT
						| IRQ_BAT_HOT_COLD_SOFT_BIT
						| IRQ_BAT_HOT_COLD_HARD_BIT);
	if (rc < 0)
		pr_err("Couldn't set irq_cfg rc=%d\n", rc);

	/* ZTE-MODIFY By jiangzhineng, P816S05's BMD pin is NOT used */
	#if 1
	rc = smb1360_write(chip, IRQ2_CFG_REG, IRQ2_VBAT_LOW_BIT
						| IRQ2_POWER_OK_BIT);
	#else
	...
	/* ZTE-MODIFY BY DingXuehua jiangzhineng, enable soc change bit of IRQ*/
	#if 1
...
static int smb1360_charging_enable(struct smb1360_chip *chip)
{
	int rc;
...
static void smb1360_shutdown(struct i2c_client *client)
{
	int rc;
	struct smb1360_chip *chip = i2c_get_clientdata(client);
       //ZTE_MODIFIED by wangbin to enable charge before shutdown
       //if(charging_disabled)
	   	//smb1360_charging_disable(chip,USER,false);

	//ZTE-MODIFY By DingXuehua, always enable charging before shutdown
	rc = smb1360_charging_enable(chip);
	if (rc < 0)
