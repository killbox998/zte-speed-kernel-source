...
static void do_taos_work(struct work_struct *w)
{
	...
	if(1 == prox_in_sun_flag) {
		printk("taos,prox_in_sun-->get als data, interrupt\n");
		ret = taos_get_lux();
		if(ret >= 0){
			g_nlux = ret;
		}
	...
	if((status & 0x10) != 0)
    {
     printk("taos, interrupt status OK-->get als data,interrupt\n");
	 ret = taos_get_lux();
         if(ret >= 0){
             g_nlux = ret;
         }
...
static int taos_enable_als_sensor(int enable)
{
	...
	if(1 == enable) { /*enable als sensor*/
			...
			if(prox_on) {
				printk("prox on, has open light\n");
				mutex_unlock(&taos_datap->read_i2c_data);
				return ret;
			}
			ret = taos_sensor_platform_hw_power_on(true);
			if (ret < 0)
				printk("taos,%s: enable_als_sensor fail \n",__func__);
			...
			if(prox_on) {
				printk("prox on, can not close light\n");
				mutex_unlock(&taos_datap->read_i2c_data);
				return -1;
			}
...
static int taos_enable_ps_sensor(int enable)
{
	...
	if(1 == enable){ /*enable prox sensor*/
			...
			ret = taos_sensor_platform_hw_power_on(true);
			if (ret < 0)
				printk("taos,%s:  enable prox fail \n",__func__);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("taos,%s: do calibrate", __func__);
...
//		return -ENODEV;
	...
	if (clientp->dev.of_node) {
		...
		if (clientp->dev.of_node) {
			...
			printk("taos, get int_gpio: %d \n", taos_datap->pdata->int_gpio);
		}

		printk(KERN_CRIT "8610---222 taos use gpio %d  irq %d\n",taos_datap->pdata->int_gpio,taos_datap->pdata->irq);

        ret = taos_pinctrl_init(taos_datap, clientp);
		if (ret) {
			dev_err(&clientp->dev, "Can't initialize pinctrl\n");
			return ret;
		}
