...
#ifdef KXTJ9_VDD_HOLD_FOR_BOARD_TEST
		//vdd hold for board test, xym20141023 begin
		vdd_hold = regulator_get(&data->client->dev, "vdd");
		if (IS_ERR(vdd_hold)) {
			rc = PTR_ERR(vdd_hold);
			dev_err(&data->client->dev,
...
static DEVICE_ATTR(cali, 0666, kxtj9_attr_get_cali, kxtj9_attr_set_cali); //0664 is ok, too.
static DEVICE_ATTR(cali_data, 0666, kxtj9_attr_get_cali_data,  kxtj9_attr_set_cali_data); 
#endif
...
{
	dev_info(&client->dev, "%s: return -38\n", __func__);
	return -ENOSYS;
}
