...
static bool done_boot_calibrate = false;
...
static int32_t stk3x1x_enable_ps(struct stk3x1x_data *ps_data, uint8_t enable)
{
    ...
    if(enable)
	{
		...
		stk3x1x_set_ps_thd_h(ps_data, ps_data->ps_thd_h);
		stk3x1x_set_ps_thd_l(ps_data, ps_data->ps_thd_l);
		printk(KERN_INFO "%s: in enable prox to set hight = %d, low = %d\n", __func__, ps_data->ps_thd_h, ps_data->ps_thd_l);
#ifdef STK_POLL_PS
...
static int stk_ps_val(struct stk3x1x_data *ps_data)
{
	...
	//de = (ps_data->psctrl_reg) & 0x3F; 
	mode = (ps_data->pdata->psctrl_reg) & 0x3F;
	if(mode == 0x30)	
		lii = 100;	
	else if (mode == 0x31)
		lii = 200;		
...
static int stk_ps_tune_zero_final(struct stk3x1x_data *ps_data)
{
	...
	stk3x1x_set_ps_thd_h(ps_data, ps_data->ps_thd_h);
	stk3x1x_set_ps_thd_l(ps_data, ps_data->ps_thd_l);
	done_boot_calibrate = true;
	printk(KERN_INFO "%s: boot calibrate to set HT=%d,LT=%d\n", __func__, ps_data->ps_thd_h,  ps_data->ps_thd_l);		
	hrtimer_cancel(&ps_data->ps_tune0_timer);					
	return 0;
}
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_data *ps_data)
{
	...
	printk(KERN_INFO "%s: auto prox calibrate --> high = %d, low = %d \n", __func__, ps_data->ps_thd_h, ps_data->ps_thd_l);
...
//add note
static ssize_t stk_show_calibrate(struct device *dev,
				struct device_attribute *attr, char *buf)
{
#if 0
	printk("taos: hi=%d,pulse cnt=%d,gain=%d\n", taos_cfgp->prox_threshold_hi,
	...
	return sprintf(buf, "%d,%d,%d\n", taos_cfgp->prox_threshold_hi,
									...
									taos_cfgp->prox_gain);
#endif
/*reg 0x1==  PRS_PS[1:0] ( Bit[7:6] )   //interrupt persistence filter   ==> psctrl_reg value
                    GAIN_PS[1:0] ( Bit[5:4] )   //Gain setting for PS
                        IT_PS[3:0] ( Bit[3:0] )   //refresh time of PS

   reg 0x3==    IRDR[1:0] ( Bit[7:6] )   //different driving, current  ==> ledctrl_reg value
                   DT_LED[5:0] ( Bit[5:0] )   //driving ON-duty (with respect to refresh time), depend on IT_PS[3:0]
*/

	struct stk3x1x_data *ps_data = dev_get_drvdata(dev);

	printk("%s data: hi=%d, pulse cnt=%d, gain = %d\n", __func__, ps_data->ps_thd_h,
								ps_data->pdata->psctrl_reg,
								ps_data->pdata->ledctrl_reg);

	return sprintf(buf, "--%d,%d,%d\n",ps_data->ps_thd_h,4,0x60);

}

static ssize_t stk_store_calibrate(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for(cnt = 0; cnt < 20; cnt ++) {
		msleep(60);
		ps_adc = stk3x1x_get_ps_reading(ps_data);
		printk(KERN_INFO "%s: ps_adc =%d , count = %d\n", __func__, ps_adc, cnt);
	...
	printk(KERN_INFO "%s: data[0]=%d, data[2]=%d\n", __func__, ps_data->ps_stat_data[0], ps_data->ps_stat_data[2]);
	printk(KERN_INFO "%s: average=%d, high=%d, low=%d\n", __func__, ps_data->ps_stat_data[1], ps_data->ps_thd_h,  ps_data->ps_thd_l);
...
static ssize_t stk_store_set_calibrate_data(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int i, j;
	uint16_t data_tmp;
	...
	u8 prox_pulse_cnt = 0;
	u8 prox_gain = 0;
	char cal_data_char[4][20];
	char buf_data[100];
	char *tmp = buf_data;

	struct stk3x1x_data *ps_data = dev_get_drvdata(dev);

	printk("user insert buf:%s\n", buf); //snprintf(set_cal_data, sizeof(set_cal_data), "%d,%d,%d,%d", threshold_hi, pulse_cnt,prox_gain_data,prox_threshold_lo);
	sprintf(buf_data, "%s\n", buf);
	printk("user insert buf_data:%s\n", buf_data);
	...
	printk("transform to number: high:%d, pluse:%d, gain:%d, low:%d\n", 
			prox_threshold_hi,prox_pulse_cnt, prox_gain, prox_threshold_lo);

	//ps_data->ps_thd_h = ps_data->ps_stat_data[1] + ps_data->stk_ht_n_ct;
	//ps_data->ps_thd_l = ps_data->ps_stat_data[1] + ps_data->stk_lt_n_ct;

	data_tmp = prox_threshold_hi - ps_data->stk_ht_n_ct;
	ps_data->ps_thd_l = data_tmp + ps_data->stk_lt_n_ct;
	ps_data->ps_thd_h = prox_threshold_hi;

	printk("last get : high:%d, low:%d\n", 
...
err_als_sysfs_create_group:
	sysfs_remove_group(&client->dev.kobj, &stk_attr_group);
