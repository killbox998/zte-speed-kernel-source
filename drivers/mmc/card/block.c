...
if(rqc)
	DBG("ygl :%s :%s:[%s], [0x%x], [%d]\n", __func__, req->rq_disk->disk_name,
	                        rq_data_dir(rqc) == WRITE ? "write":"read", (unsigned int)blk_rq_pos(rqc), blk_rq_sectors(rqc));
...
//yeganlin_20140102,set system device to ro if it was write protected
	if(mmc_card_mmc(card)){
		 ...
		 *  ZTE_20141024
		 */
		set_partition_ro(card,md,"sbl1",7); //sbl1+sblbak+DDR+rpm++rpmbak+tz+tzbak,
		set_partition_ro(card,md,"aboot",6); //aboot+abootbak+modem+boot+recovery+splash
...
//yeganlin,add for debug
#ifndef CONFIG_MMC_DEBUG
static int sdhci_block_proc_show(struct seq_file *file, void *v)
{

	//int len = 0;
	//len = sprintf(page, "%s\n",
      //                !!block_debug == 1?"on":"off");
      pr_err("ygl ::%s,e\n",__func__);
...
static int sdhci_block_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, sdhci_block_proc_show, NULL);
}

static ssize_t sdhci_block_write(struct file *file, const char __user *buf,
...
static const struct file_operations sdhci_block_proc_fops = {
	.open		= sdhci_block_proc_open,
	.read		= seq_read,
	.write		= sdhci_block_write,
	.release	      = single_release,	
};

static void
init_block_proc(void)
{
	proc_create("msm_block_log", 0644, NULL, &sdhci_block_proc_fops);
}
#endif
